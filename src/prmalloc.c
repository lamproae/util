...
struct pginfo {
    ...
    unsigned long		bits[1]; /* Which chunks are free */
};
...
struct pgfree {
    ...
    unsigned long		size;	/* number of bytes free */
};

/*
 * How many bits per unsigned long in the bitmap.
 * Change only if not 8 bits/byte
 */
#define	MALLOC_BITS	(8*sizeof(unsigned long))
 ...
 */
static	unsigned long  malloc_origo;

/*
 * The last index in the page directory we care about
 */
static	unsigned long  last_index;
...
static int extend_page_directory(unsigned long index);
...
malloc_dump(FILE *fd)
{
    ...
    fprintf(fd,"Break\t%ld\n",(unsigned long)sbrk(0) >> malloc_pageshift);
}
...
map_pages(int pages, int update)
{
    ...
    result = (caddr_t) ((unsigned long)result & ~malloc_pagemask);
    tail = result + (pages << malloc_pageshift);
    if (!brk(tail)) {
        last_index = ((unsigned long)tail >> malloc_pageshift) - malloc_origo -1;
        malloc_brk = tail;
 ...
 */
static int
extend_page_directory(unsigned long index)
{
    ...
    /* Mark the pages */
    index = ((unsigned long)young >> malloc_pageshift) - malloc_origo;
...
{
    unsigned long index = ((unsigned long)ptr >> malloc_pageshift) - malloc_origo;
...
malloc_init (void)
{
    ...
    for(;;) {
        ...
        if (sizeof(struct pginfo) + j - sizeof (unsigned long) <= i)
            break;
        i += i;
    }
    malloc_minsize = i;
     ...
     */
    malloc_origo = (unsigned long) page_dir >> malloc_pageshift;
...
static void *malloc_pages(size_t size)
{
    ...
    for(pf = free_list.next; pf; pf = pf->next) {
        ...
        if (page_dir[((unsigned long)pf->page >> malloc_pageshift) - malloc_origo] 
                != MALLOC_FREE) {
            TRACE(("%6d !f %p %p %p <%d>\n",malloc_event++,
                        pf,pf->page,pf->end,__LINE__));
            wrterror("non-free first page on free-list\n");
        }
        if (page_dir[((unsigned long)pf->end >> malloc_pageshift) - 1 - malloc_origo] 
                != MALLOC_FREE)
...
#ifdef EXTRA_SANITY
    if (p && page_dir[((unsigned long)p >> malloc_pageshift) - malloc_origo] 
            != MALLOC_FREE) {
    ...
    if (p) {
        ...
        index = ((unsigned long)p >> malloc_pageshift) - malloc_origo;
        page_dir[index] = MALLOC_FIRST;
        for (i=1;i<size;i++)
...
malloc_make_chunks(int bits)
{
    ...
    if (!pp)
        return 0;
    l = sizeof *bp - sizeof(unsigned long);
...
void *_PR_UnlockedMemalign(size_t alignment, size_t size)
{
    ...
    if ((unsigned long)result & (alignment - 1))
        return ((void *)(((unsigned long)result + alignment)  & ~(alignment - 1)));
    else
        return result;
}
...
{
    void *p;
    unsigned long osize,page,index,tmp_index;
    struct pginfo **mp;
    ...
    page = (unsigned long)ptr >> malloc_pageshift;
...
static void
free_pages(char *ptr, unsigned long page, int index, struct pginfo *info)
{
    int i;
    struct pgfree *pf,*pt;
    unsigned long l;
    ...
    if ((unsigned long)ptr & malloc_pagemask)
    ...
    if (!pf->next &&
        ...
        malloc_brk = pf->end;
        /* Find the page directory entry for the page in question */
        page = (unsigned long)pf->end >> malloc_pageshift;
        index = page - malloc_origo;
...
static void
free_bytes(void *ptr, unsigned long page, int index, struct pginfo *info)
{
    int i;
    struct pginfo **mp;
    void *vp;
...
#ifdef SANITY
    if ((unsigned long)ptr & (info->size - 1))
        wrterror("freeing messed up chunk pointer\n");
#endif

    /* Find the chunk number on the page */
    i = ((unsigned long)ptr & malloc_pagemask) >> info->shift;
