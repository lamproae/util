...
smap_add(struct smap *smap, const char *key, const char *value)
{
    ...
    return smap_add__(smap, memdup0(key, key_len), strdup(value),
...
smap_add_format(struct smap *smap, const char *key, const char *format, ...)
{
    ...
    value = vasprintf(format, args);
    va_end(args);

    key_len = strlen(key);
    smap_add__(smap, memdup0(key, key_len), value,
               hash_bytes(key, key_len, 0));
}
...
smap_replace(struct smap *smap, const char *key, const char *value)
{
    ...
    if (node) {
        ...
        smap_add__(smap, memdup0(key, key_len), strdup(value), hash);
    }
}
...
smap_clone(struct smap *dst, const struct smap *src)
{
    ...
    SMAP_FOR_EACH (node, src) {
        smap_add__(dst, strdup(node->key), strdup(node->value),
                   node->node.hash);
    }
}
...
{
    struct smap_node *node = malloc(sizeof *node);
    node->key = key;
    node->value = value;
