...
{
    return json_string_create_nocopy(strdup(s));
}
...
json_array_trim(struct json *array_)
{
    ...
    if (array->n < array->n_allocated){
        ...
        array->elems = realloc(array->elems, array->n * sizeof *array->elems);
    }
}
...
{
    struct json **elems = malloc(sizeof *elems);
    elems[0] = elem0;
...
{
    struct json **elems = malloc(2 * sizeof *elems);
    elems[0] = elem0;
    elems[1] = elem1;
...
{
    struct json **elems = malloc(3 * sizeof *elems);
    elems[0] = elem0;
    elems[1] = elem1;
    elems[2] = elem2;
...
json_object_create(void)
{
    ...
    json->u.object = malloc(sizeof *json->u.object);
...
json_clone_array(const struct json_array *array)
{
    ...
    elems = malloc(array->n * sizeof *elems);
    for (i = 0; i < array->n; i++) {
...
json_from_file(const char *file_name)
{
    ...
    if (!stream) {
        return json_string_create_nocopy(
            asprintf("error opening \"%s\": %s", file_name,
                      strerror(errno)));
    }
...
json_from_stream(FILE *stream)
{
    ...
    if (ferror(stream)) {
        ...
        json = json_string_create_nocopy(
            asprintf("error reading JSON stream: %s", strerror(errno)));
    }
...
{
    struct json_parser *p = zalloc(sizeof *p);
    p->flags = flags;
    return p;
}
...
json_parser_input(struct json_parser *p, struct json_token *token)
{
    ...
    case JSON_PARSE_OBJECT_NAME:
        if (token->type == T_STRING) {
            p->member_name = strdup(token->u.string);
...
{
    struct json *json = malloc(sizeof *json);
    json->type = type;
    json->count = 1;
    return json;
}
